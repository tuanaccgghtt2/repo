...
// }
// $('head').append('<link rel="stylesheet" href="/css/'+theme+'.css?_=f" type="text/css" />');
...
// 	  if(this.id=="dnt") {
// 		  $("#dnt_txt").html("Bạn có thể ủng hộ USD qua thư PayPal: "+"@"+"gmail.com");
// 	  }
//   });
// });
function loadPackageInfo() {
	...
	if (navigator.userAgent.search(/Cydia/) == -1) {
		$("#showAddRepo_").show();
		$("#open_cydia").attr("href","cydia://url/https://cydia.saurik.com/api/share#?source=https://repo.tuandb.name.vn/&package="+urlSelfParts[1]);
	}
	$.ajax({
		url: form_url,
		type: "GET",
...
{
	var sectionContent = "";
	sectionContent += "<li class=\"has-icon\"><a onclick=\"loadMainSection()\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"/img/back.png\" alt=\"\" srcset=\"/img/back.png 2x, /img/back.png 3x\" class=\"icon\"/><label><< Back</label></a></li>";
	for (var keyNow in packagesSection[sectionName]) {
		...
		if (navigator.userAgent.search(/Cydia/) == -1) {
			var current_url_path = window.location.href.slice(0, window.location.href.lastIndexOf('/'));
			urlOpen = current_url_path+"/info.html?id="+dicNow.package;
		}		
		sectionContent +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"/img/"+encodeURI(sectionName)+".png\" alt=\"\" srcset=\"/img/"+encodeURI(sectionName)+".png 2x, /img/"+encodeURI(sectionName)+".png 3x\" class=\"icon\"/><label>"+dicNow.name+" v"+dicNow.version+"</label></a></li>";
	}
...
function loadMainSection()
{
	...
	for (var section in packagesSection) {		
		sectionContent += "<li class=\"has-icon\"><a onclick=\"openSection('"+section+"')\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"/img/"+encodeURI(section)+".png\" alt=\"\" srcset=\"/img/"+encodeURI(section)+".png 2x, /img/"+encodeURI(section)+".png 3x\" class=\"icon\"/><label>"+section+" ("+packagesSection[section].length+")</label></a></li>";
	}
	$("#browser").html(sectionContent);
}
...
function loadRecentUpdates()
{
	...
	for (var dicNow in allPackages) {
		...
		if (navigator.userAgent.search(/Cydia/) == -1) {
			var current_url_path = window.location.href.slice(0, window.location.href.lastIndexOf('/'));
			urlOpen = current_url_path+"/info.html?id="+allPackages[dicNow].package;
		}				
		htmlnews +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"/img/"+encodeURI(allPackages[dicNow].section)+".png\" alt=\"\" srcset=\"/img/"+encodeURI(allPackages[dicNow].section)+".png 2x, /img/"+encodeURI(allPackages[dicNow].section)+".png 3x\" class=\"icon\"/><label>"+allPackages[dicNow].name+" v"+allPackages[dicNow].version+"</label></a></li>";
	}
